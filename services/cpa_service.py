...
def _payload_access_token(payload: dict) -> str:
    return str(payload.get("access_token") or payload.get("accessToken") or "").strip()
...
def fetch_remote_account_payload(pool: dict, file_name: str) -> tuple[dict | None, str | None]:
    ...
    access_token = _payload_access_token(payload)
    if not access_token:
    ...
    def _run_import(self, pool_id: str, pool: dict, names: list[str]) -> None:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_map):
                ...
                if account:
                    account_payloads.append(account)
                else:
        ...
        tokens = [_payload_access_token(item) for item in account_payloads]
